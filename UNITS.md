...
✅ **Verified.** `GTValues.L = 144`, documented in source as "Fluid per Material Unit" (`GTValues.java:80`; now deprecated in favour of `GTRecipeBuilder.INGOTS`, same ratio). The propagation from fluids to solids therefore holds, and the 1.134 kg iron ingot stands as computed. Its melt duty has since been recomputed by integration — see below.

⚖️ **Rationale.** This is the only basis under which one number means the same thing for a fluid and a solid, and under which volume, mass and moles all follow from one declaration. It also reproduces GTNH's existing pacing: the melt duty for one iron ingot is **≈ 1.47 MJ** (1191 kJ sensible from 298→1809 K, plus 280 kJ fusion), which at HV takes **≈ 144 s** and at EV ≈ 36 s — the same order as stock blast furnace timings. A molar basis (1000 mB ≡ 1 mol) makes the same ingot 8 g and the same melt about 1 s at HV, which would force artificially inflated durations to stay recognizable — the "artificial grind" PHILOSOPHY §5 forbids.

⚠️ **This number moved, and the correction is worth recording.** It was first stated as 1.05 MJ and ≈ 103 s, computed as Cp × ΔT with a single heat capacity measured at 298 K. Integrating NIST's Shomate correlation over 298→1809 K instead gives 58.6 kJ/mol, **40 % more** than the constant-value estimate, because iron's heat capacity rises steeply with temperature and the magnetic transition near 1042 K is invisible to a single point. The pacing argument survives — 144 s is still squarely in the range of stock blast furnace timings, and the molar basis is still off by two orders of magnitude — but the figure the matter basis was justified on was understated, and the justification now rests on an integral rather than an approximation.
